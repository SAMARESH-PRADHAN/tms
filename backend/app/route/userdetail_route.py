...
def get_users():
    try:
        sql = text("SELECT user_id, user_name, email, is_active FROM public.user_m")
...
def add_user():
    ...
    sql = text("""
        INSERT INTO public.user_m (user_name, email, password, mobile, address, role_id, created_by, updated_by, is_active)
...
def update_user(user_id):
    data = request.json
    sql = text("""
        UPDATE public.user_m SET user_name = :user_name, email = :email WHERE user_id = :user_id
    """)
    try:
...
def update_user_status(user_id):
    ...
    sql = text("""
        UPDATE public.user_m SET is_active = :is_active WHERE user_id = :user_id
    """)
    try:
